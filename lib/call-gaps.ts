...
const KEYS = {
  ...
  // Set of all user IDs that have gap data for a given date
...
export async function recordCallEnded(
  ...
  pipeline.set(KEYS.lastCallEnded(aircallUserId), lastEnded, { ex: 86400 }) // 24h TTL

  // Track this rep as active today
  pipeline.sadd(KEYS.activeReps(date), `${aircallUserId}:${repName}`)
  pipeline.expire(KEYS.activeReps(date), 172800) // 48h TTL
  ...
  pipeline.expire(KEYS.callEvent(aircallUserId, date), 7776000) // 90 days

  // Update daily summary call counts
...
export async function recordCallStarted(
  ...
  // Track this rep as active today
  await redis.sadd(KEYS.activeReps(date), `${aircallUserId}:${repName}`)
  await redis.expire(KEYS.activeReps(date), 172800)
  ...
  await redis.rpush(KEYS.callEvent(aircallUserId, date), JSON.stringify(event))
  await redis.expire(KEYS.callEvent(aircallUserId, date), 7776000)
  ...
  // Store the gap
  await redis.rpush(KEYS.dailyGaps(aircallUserId, date), JSON.stringify(gap))
  await redis.expire(KEYS.dailyGaps(aircallUserId, date), 31536000) // 12 months
...
export async function getGapsToday(): Promise<LiveRepGap[]> {
  ...
  for (const member of members) {
    const [userIdStr, ...nameParts] = (member as string).split(':')
    const userId = parseInt(userIdStr, 10)
    const repName = nameParts.join(':')

    const [summary, lastEnded] = await Promise.all([
    ...
    results.push({
      ...
      rep_name: repName || summary?.rep_name || 'Unknown',
  ...
  return results.sort((a, b) => {
    // Reps with gaps first, then by avg gap ascending
...
export async function getLiveIdleData(): Promise<{ aircall_user_id: number; rep_name: string; idle_seconds: number; last_call_ended_at: number }[]> {
  ...
  for (const member of members) {
    ...
    const lastEnded = await redis.get<LastCallEnded>(KEYS.lastCallEnded(userId))
    if (!lastEnded) continue

    const idleSeconds = Math.max(0, now - lastEnded.ended_at)
    if (idleSeconds > MAX_GAP_THRESHOLD) continue // Skip if idle too long (not working)

    results.push({
      aircall_user_id: userId,
      rep_name: repName || lastEnded.rep_name,
...
/**
 * Get team-wide average gap for today.
 */
export async function getTeamAvgGap(): Promise<{ avg_gap_seconds: number; total_reps: number; total_gaps: number }> {
...
function toDateString(unixTimestamp: number): string {
  // Use UK timezone for date grouping
