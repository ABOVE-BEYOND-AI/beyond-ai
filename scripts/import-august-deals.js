...
Gross: £6,017.94
Special Requirements: CREDIT NOTE TOTALLING £2,666.20 FROM LFW HAS BEEN ALLOCATED TOWARDS THIS BOOKING
Payment Terms: 10% deposit - 40% in 7 days - 50% 16 weeks before the event date
Sold By: Sam Harrop
  ...
  dealBlocks.forEach((block, index) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('Gross: £')) {
        grossAmount = line.replace('Gross: £', '').replace(/,/g, '');
        continue;
      }
      ...
      if (!clientName && line && !line.includes('£') && !line.includes(':')) {
        clientName = line;
        continue;
      }
      
      // If we have client name but no event name yet
      if (clientName && !eventName && line && !line.includes('£') && !line.includes(':') && line !== clientName) {
        eventName = line;
        continue;
      }
    }
...
async function importDeals() {
  ...
  deals.forEach((deal, index) => {
    ...
    console.log(`   Amount: £${(deal.amount / 100).toLocaleString('en-GB', { minimumFractionDigits: 2 })}`);
    console.log('');
  });
  ...
  for (let i = 0; i < deals.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ ${i + 1}/${deals.length} - ${deal.deal_name} (${deal.rep_name})`);
      } else {
        console.error(`❌ ${i + 1}/${deals.length} - Failed: ${deal.deal_name}`, await response.text());
      }
    } catch (error) {
      console.error(`❌ ${i + 1}/${deals.length} - Error: ${deal.deal_name}`, error.message);
    }
  ...
  console.log('');
  console.log('🎉 Import complete! Check your dashboard at:');
